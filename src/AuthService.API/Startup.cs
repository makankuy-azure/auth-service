...
namespace AuthService.API
{
    class Startup : FunctionsStartup
    {
        public override void Configure(IFunctionsHostBuilder builder)
        {
            ...
            if (!builder.Services.BuildServiceProvider().GetService<IConfiguration>().GetSection(nameof(AccessTokenHandler.Settings.AccessTokenSettings)).Exists()) 
                throw new ArgumentNullException($"'{nameof(AccessTokenHandler.Settings.AccessTokenSettings)}' configuration section is required.");

            builder.Services.AddSingleton<AccessTokenHandler.Settings.IAccessTokenSettings>(sp =>
                sp.GetService<IConfiguration>().GetSection(nameof(AccessTokenHandler.Settings.AccessTokenSettings)).Get<AccessTokenHandler.Settings.AccessTokenSettings>());
            ...
            serviceProvider.GetService<AccessTokenHandler.Settings.IAccessTokenSettings>().ValidateAttributes();
